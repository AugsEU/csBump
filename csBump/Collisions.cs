namespace csBump
{
	...
	public class Collisions : IComparer<int>
	{
		...
		public int Compare(int a, int b)
		{
			if (mTIs[a] == (mTIs[b]))
			{
				float ad = Rect2f.GetSquareDistance(new Rect2f(mX1s[a], mY1s[a], mW1s[a], mH1s[a]), new Rect2f(mX2s[a], mY2s[a], mW2s[a], mH2s[a]));
				float bd = Rect2f.GetSquareDistance(new Rect2f(mX1s[a], mY1s[a], mW1s[a], mH1s[a]), new Rect2f(mX2s[b], mY2s[b], mW2s[b], mH2s[b]));
